#!/bin/bash
# Build and deploy inServ to minikube cluster (inOrch profile)
...
echo "=== Building and deploying inServ to minikube (profile: $PROFILE) ==="
...
helm upgrade inserv charts/inServ \
  --namespace $NAMESPACE \
...
kubectl rollout restart deployment/inserv-inserv -n $NAMESPACE

# Step 8: Wait for rollout to complete
echo ""
echo "Step 8: Waiting for deployment rollout..."
kubectl rollout status deployment/inserv-inserv -n $NAMESPACE --timeout=300s

# Step 9: Verify the deployment
echo ""
echo "Step 9: Deployment status:"
kubectl get pods -n $NAMESPACE -l app.kubernetes.io/name=inserv

# Step 10: Verify new image is running
echo ""
echo "Step 10: Verifying new image is running in pod..."
# Get the running pod (not terminating ones) - filter by phase=Running
POD_NAME=$(kubectl get pods -n $NAMESPACE -l app.kubernetes.io/name=inserv --field-selector=status.phase=Running -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
if [ -z "$POD_NAME" ]; then
...
echo "  kubectl logs -n $NAMESPACE -l app.kubernetes.io/name=inserv --tail=50 -f"
echo ""
