...
async def lifespan(app: FastAPI):
    ...
    async with _mcp.lifespan(app):
        await run_pending_sql_migrations()
        await db.connect()
...
async def register_agent_simple(registration: AgentRegister, request: Request):
    ...
    # Optional duplicate-by-hostname policy. Disabled by default so one host
    # can serve multiple agents under different paths.
    if settings.enforce_unique_hostname:
