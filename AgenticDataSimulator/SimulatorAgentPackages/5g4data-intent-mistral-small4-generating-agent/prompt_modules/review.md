...
- For each expectation kind you plan to include, confirm a matching observation-reporting block with per-anchor event and duration URIs (not global `TenMinuteReportEventDeployment` / `tenMinutesDeployment`). Skip reporting sections for omitted expectation kinds.
