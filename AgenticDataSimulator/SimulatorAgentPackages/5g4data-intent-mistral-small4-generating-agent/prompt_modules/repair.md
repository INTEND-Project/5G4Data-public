...
- If Intent has more than one `dct:description` or contains `log:forAll` / `set:forAll`, move condition predicates into separate `data5g:CO… a log:Condition` blocks and leave Intent with a single summary `dct:description` plus `log:allOf` only.
