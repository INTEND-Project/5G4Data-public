...
exports.GraphDbTool = exports.NEAREST_EDGE_DATACENTER_QUERY = void 0;
exports.NEAREST_EDGE_DATACENTER_QUERY = `
...
class GraphDbTool {
    ...
    namedGraph;
    queryLimit;
    constructor(endpoint, namedGraph, queryLimit) {
    ...
    async nearestEdgeCandidates() {
        const response = await fetch(this.endpoint, {
            method: "POST",
            headers: { Accept: "application/sparql-results+json", "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams({ query: this.buildQuery(exports.NEAREST_EDGE_DATACENTER_QUERY) }).toString()
        });
        if (!response.ok) {
    ...
    async getIntentTurtle(intentId) {
        const root = this.intentRootUri(intentId);
        if (!root)
            return null;
        // Forward-only closure from the intent root (see OpenClawController `buildIntentConstructSparql`).
        const query = `
...
WHERE {
  ...
  FILTER( ?s = <${root}> || !REGEX(STR(?s), "^http://5g4data\\\\.eu/5g4data#I[0-9a-f]{32}$", "i") )
}
`.trim();
...
exports.GraphDbTool = GraphDbTool;
