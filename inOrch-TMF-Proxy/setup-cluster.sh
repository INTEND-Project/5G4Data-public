...
# Enable ingress
kubectl label node inorch-tmf-proxy minikube.k8s.io/primary=true --context $PROFILE
...
kubectl -n inorch-tmf-proxy create secret docker-registry ghcr-creds \
  ...
  --docker-password=ghp_bWxRZH0z97YK0ZKdJZbF91f9a3X2Z4 \
  --docker-email=you@example.com


# Not sure if the DNS stuff is needed....
exit 1
...
echo ""
echo "2. Run ./build-and-deploy.sh to deploy inOrch-TMF-Proxy"
echo ""
echo "3. Create ghcr-creds secret in inorch-tmf-proxy namespace:"
echo "   kubectl -n inorch-tmf-proxy create secret docker-registry ghcr-creds \\"
