...
## SimulatorAgentKernel and SimulatorAgentPackages concept

`SimulatorAgentKernel` is the generic runtime kernel, while `SimulatorAgentPackages` holds domain-specific behavior as installable packages. Instead of building a new agent codebase for each domain, teams assemble domain agents by attaching a package to the same core runtime.
...
- **Lower maintenance cost:** Core improvements happen once in `SimulatorAgentKernel` and benefit every package-based agent.
...
With this model, creating a domain-specific agent is primarily a packaging task: define the domain behavior in `SimulatorAgentPackages/<package-name>`, load it into the kernel, and run the resulting `SimulatorAgentKernel-<package-name>` instance.
