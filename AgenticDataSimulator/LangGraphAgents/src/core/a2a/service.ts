...
import { buildAgentSecurityConfig } from "./auth.js";
import type { SecurityScheme } from "./auth.js";
...
export function buildAgentCard(config: A2AConfig, domainPackage: LoadedDomainPackage): AgentCard {
  ...
  const card: AgentCard = {
    ...
    name: cardName,
    description: partial?.description ?? `LangGraphAgents package runtime for ${packageName}`,
  ...
  const apiKey = config.agentApiKey?.trim();
  if (apiKey) {
    const auth = buildAgentSecurityConfig(apiKey, {
      headerName: config.agentApiKeyHeader
    });
    card.securitySchemes = auth.securitySchemes;
    card.security = auth.security;
  }
  return card;
}
