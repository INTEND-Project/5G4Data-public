...
test("buildAgentCard includes securitySchemes when agentApiKey configured", () => {
  const card = buildAgentCard(
    {
      ...baseConfig,
      agentApiKey: "test-secret",
  ...
  assert.ok(card.securitySchemes?.["agent-api-key"]);
  assert.deepEqual(card.security, [{ "agent-api-key": [] }]);
});
