...
test("OpenAPI server handles sessions, turns, and agent card", async () => {
  const runtime = {
    ...
    getDomainPackage() {
      return { packageDir: "/tmp/pkg", manifest: { name: "demo", version: "0.1.0" } };
    },
    getAppConfig() {
...
test("JSON-RPC listens on /v1 when advertised card.url pathname includes proxy prefix", async () => {
  ...
  const agentCard: AgentCard = {
    ...
    name: "demo",
    description: "demo",
    url: "https://example/5g4data-intent-langgraph-generating-agent/v1",
    version: "0.1.0",
    capabilities: { streaming: false, pushNotifications: false, stateTransitionHistory: false },
    defaultInputModes: ["text/plain"],
    defaultOutputModes: ["text/plain"],
  ...
  try {
    const baseUrl = `http://127.0.0.1:${address.port}`;
    const prefixed = `${baseUrl}/5g4data-intent-langgraph-generating-agent/v1`;
