...
data5g:CX__ID_CONTEXT_1__ a icm:Context ;
    data5g:Application "<chart-name-from-catalogue>" ;
    data5g:DataCenter "<data-center>" ;
    data5g:DeploymentDescriptor "<catalogue-deployment-descriptor-url>" .

data5g:CX__ID_CONTEXT_REGION_1__ a icm:Context ;
    data5g:appliesToCustomer "<customer>" ;
    data5g:appliesToRegion data5g:RG__ID_REGION_1__ .
...
data5g:DE__ID_DEPLOYMENT_1__ a data5g:DeploymentExpectation,
    ...
    log:allOf data5g:CO__ID_CONDITION_1__,
        data5g:CX__ID_CONTEXT_1__ .

data5g:NE__ID_NETWORK_1__ a data5g:NetworkExpectation,
        icm:Expectation,
        icm:IntentElement ;
    dct:description "<network expectation description>" ;
    icm:target data5g:network-slice ;
    log:allOf data5g:CO__ID_CONDITION_BANDWIDTH_1__,
        data5g:CO__ID_CONDITION_LATENCY_1__,
        data5g:CX__ID_CONTEXT_1__ .
...
data5g:SE__ID_SUSTAINABILITY_1__ a data5g:SustainabilityExpectation,
    ...
    log:allOf data5g:CO__ID_CONDITION_2__,
        data5g:CX__ID_CONTEXT_1__ .
...
data5g:RE__ID_REPORT_DEPLOYMENT_1__ a icm:ObservationReportingExpectation ;
...
data5g:RE__ID_REPORT_SUSTAINABILITY_1__ a icm:ObservationReportingExpectation ;
...
data5g:RE__ID_REPORT_NETWORK_1__ a icm:ObservationReportingExpectation ;
...
Illustrative shape only (metric names vary per chart):
...
Use metric `data5g:<objective-name>___ID_CONDITION_<LABEL>_1__` (same placeholder token as the condition; postprocessing rewrites to `data5g:<objective-name>_CO…` with a UUIDv4 suffix). Emit `quan:<op> [ quan:unit "<unit>" ; rdf:value <threshold> ]` from catalogue hints unless the user overrides.
...
Use metric `data5g:<metric-name>___ID_CONDITION_<LABEL>_1__` (postprocessing rewrites to `data5g:<metric-name>_CO…`). Emit thresholds from catalogue hints unless the user overrides. Prefer canonical catalogue metric names when present; treat deprecated legacy names as aliases only when they appear in the selected chart.
...
- Condition-scoped `icm:valuesOfTargetProperty`: `data5g:<metric-stem>___ID_CONDITION_<LABEL>_1__` — reuse the exact same `__ID_CONDITION_<LABEL>_1__` token as the condition; do **not** insert `CO` before the placeholder during generation. Package postprocessing rewrites this to `data5g:<metric-stem>_CO…`.
...
- Final form after postprocessing: `data5g:<metric-stem>_CO…` where `<metric-stem>` is the catalogue objective or sustainability metric `name`.

## Validation checklist

Before returning:
- Root is `icm:Intent`.
- Root `log:allOf` references only expectations and observation-reporting blocks actually present (deployment-only, network-only, sustainability-only, and mixed shapes are all valid—never require DE+SE+NE together).
...
- Network conditions use network metric stems from the prompt or catalogue (commonly bandwidth/latency-style metrics).
