            ...
            if event_type == "agent.message":
                ...
                if error_type == "model_rate_limited_error":
                    print(
                        ...
                        "Please wait a bit and send your message again.\n"
                    )
                    return False
...
def send_and_await_with_backoff(
    ...
    success = await_and_print_agent_turn(
        client,
        session_id,
        poll_interval_seconds,
        turn_timeout_seconds,
    ...
    for idx, delay in enumerate(retry_delays_seconds, start=1):
        print(
            f"Retrying same turn after rate limit "
    ...
    print("Turn still rate-limited after retries. Please try again in a minute.")
    return False
