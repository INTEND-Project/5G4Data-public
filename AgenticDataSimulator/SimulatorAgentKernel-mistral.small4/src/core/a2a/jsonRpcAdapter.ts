...
function buildCompletedTaskResult(options: {
  ...
  if (
    ...
    options.shaclConforms !== undefined ||
    options.graphdbPersisted !== undefined
  ) {
    result.metadata = {
      openclaw: {
        agentTraceVersion: "1",
        ...
        ...(options.shaclConforms !== undefined
          ? { shaclConforms: options.shaclConforms }
          : {}),
        ...(options.graphdbPersisted !== undefined
          ? { graphdbPersisted: options.graphdbPersisted }
          : {})
      }
    };
  }
...
export class A2AJsonRpcAdapter {
  ...
  async handleRawBodyAsync(rawBody: string): Promise<{ httpStatus: number; body: string }> {
    ...
    try {
      ...
      const traceTags = turn.traceTags ?? {};
      const shaclTag = traceTags["shacl.conforms"];
      const graphdbTag = traceTags["graphdb.persisted"];
      const result = buildCompletedTaskResult({
        ...
        shaclConforms:
          shaclTag === "true" || shaclTag === true
            ? true
            : shaclTag === "false" || shaclTag === false
              ? false
              : undefined,
        graphdbPersisted:
          graphdbTag === "true" || graphdbTag === true
            ? true
            : graphdbTag === "false" || graphdbTag === false
              ? false
              : undefined
      });
