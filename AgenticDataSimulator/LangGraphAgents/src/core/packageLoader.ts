...
const generationFragmentSchema = z.object({
  id: z.string().min(1),
  whenIntentFlags: z.array(z.string().min(1)).min(1),
  promptModule: z.string().min(1)
});

const generationSchema = z.object({
  mode: z.literal("fragmented"),
  canonicalPrefixesFile: z.string().min(1).optional(),
  assemblerModule: z.string().min(1),
  fragments: z.array(generationFragmentSchema).min(1)
});
...
const workflowSchema = z.object({
  confirmation: z
    .object({
      ...
      forceGenerateInstruction: z.string().min(1).optional()
    })
    .optional(),
...
export type GenerationFragmentConfig = z.infer<typeof generationFragmentSchema>;
