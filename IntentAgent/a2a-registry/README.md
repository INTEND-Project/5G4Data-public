...
## Provenance and Local Fork

This copy is maintained as an internal fork for INTEND/5G4Data-specific needs.
For upstream source details and rationale, see [`NOTICE.md`](./NOTICE.md).
...
PostgreSQL is initialized with `001_initial_schema.sql` only on **first** volume creation. Later SQL files (`002`–`006` under `backend/migrations/versions/`) are applied **automatically on startup** by the API and worker (`app/db_migrations.py`), so existing databases pick up new columns without manual `psql`. To skip this (e.g. tests), set `SKIP_SQL_MIGRATIONS=true`.
