...
export interface AppConfig {
  ...
  /** LangSmith tracing (primary observability for LangGraphAgents). */
...
export function loadConfig(): AppConfig {
  ...
  // 2) LangGraphAgents/.env (or clone .env)
  ...
  const skillFile = resolve(
    process.cwd(),
    process.env.SKILL_FILE ?? "./packages/5g4data-intent-langgraph-generating-agent/skills/SKILL.md"
  );
  const domainPackageDir = resolve(
    process.cwd(),
    process.env.DOMAIN_PACKAGE_DIR ?? "./packages/5g4data-intent-langgraph-generating-agent"
  );
  ...
  return {
    ...
    langsmithTracingEnabled: boolFromEnv(
      process.env.LANGSMITH_TRACING ?? process.env.LANGCHAIN_TRACING_V2,
      false
    ),
    langsmithApiKey: process.env.LANGSMITH_API_KEY?.trim() || undefined,
    langsmithProject: process.env.LANGSMITH_PROJECT?.trim() || undefined,
    langsmithEndpoint: process.env.LANGSMITH_ENDPOINT?.trim() || undefined
  };
}
