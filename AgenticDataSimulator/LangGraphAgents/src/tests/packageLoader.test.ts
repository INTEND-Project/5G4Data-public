...
test("loads 5g4data-intent-langgraph-generating-agent package", () => {
  const domainPackage = loadDomainPackage(basePackageDir);
  assert.equal(domainPackage.manifest.name, "5g4data-intent-langgraph-generating-agent");
...
test("domain package swap works by changing directory", () => {
  const templatePackageDir = join(
    "/home/telco/arneme/INTEND-Project/5G4Data-public/AgenticDataSimulator/LangGraphAgents/packages",
    "package-template"
  );
