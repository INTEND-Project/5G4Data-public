...
test("writeCloneDockerCompose writes docker-compose.yml to clone directory", () => {
  ...
  const composePath = writeCloneDockerCompose({
    cloneDir,
    cloneName: "5g4data-intent-langgraph-generating-agent",
    port: "3011"
  });
  assert.match(composePath, /docker-compose\.yml$/);
  const content = readFileSync(composePath, "utf8");
  assert.match(content, /container_name: 5g4data-intent-langgraph-generating-agent/);
});

test("containerNameForClone and projectNameForClone sanitize names", () => {
  assert.equal(
    containerNameForClone("5g4data-intent-langgraph-generating-agent"),
    "5g4data-intent-langgraph-generating-agent"
  );
  assert.equal(
    projectNameForClone("5g4data-intent-langgraph-generating-agent"),
    "5g4data-intent-langgraph-generating-agent"
  );
});
