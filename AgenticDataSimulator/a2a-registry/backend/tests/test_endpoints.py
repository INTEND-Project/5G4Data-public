...
def test_register_agent_host_duplicate(client):
    ...
    with patch("app.main.AgentRepository") as mock_repo, \
        ...
        mock_settings.enforce_unique_hostname = True
...
def test_register_agent_host_duplicate_allowed_when_policy_disabled(client):
    """Allow registration when hostname policy is disabled (default)."""
    mock_public = _make_agent_public()
    ...
    with patch("app.main.AgentRepository") as mock_repo, \
         ...
         patch("app.main.smoke_test", new=AsyncMock(return_value=("WORKING", "Verified working at registration."))), \
         patch("app.main.settings") as mock_settings:
        mock_settings.enforce_unique_hostname = False
        mock_settings.rate_limit_enabled = True
        mock_settings.cors_origins = ["*"]
        instance = mock_repo.return_value
        instance.get_by_well_known_uri = AsyncMock(return_value=None)
...
def test_register_agent_allows_reregister_after_soft_delete(client):
    """Allow re-register when an existing row for the URI is soft-deleted."""
    mock_public = _make_agent_public()
    ...
    with patch("app.main.AgentRepository") as mock_repo, \
         ...
         patch("app.main.smoke_test", new=AsyncMock(return_value=("WORKING", "Verified working at registration."))):
        instance = mock_repo.return_value
        # Repository now filters hidden rows; soft-deleted rows must not block registration.
        instance.get_by_well_known_uri = AsyncMock(return_value=None)
        instance.get_by_host = AsyncMock(return_value=None)
        instance.get_by_name_and_author = AsyncMock(return_value=None)
        instance.create = AsyncMock(return_value=_make_agent_in_db())
