...
async function bootstrapLangSmithTracing(
  ...
  await initializeLangSmithTracing(config, {
...
async function runPackageLoadCommand(argv: string[]): Promise<boolean> {
  ...
  const packagesRoot = resolve(baselineAgentDir, "packages");
...
if (process.argv[1]?.endsWith("index.js") || process.argv[1]?.endsWith("index.ts")) {
  ...
  const execution = (async () => {
    ...
    await bootstrapLangSmithTracing(orchestrator);
    ...
    if (config.apiServerEnabled) {
      ...
      const shutdown = async () => {
        await shutdownLangSmithTracing();
        await server.close();
        process.exit(0);
      };
