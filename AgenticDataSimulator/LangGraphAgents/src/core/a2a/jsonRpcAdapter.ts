...
function buildCompletedTaskResult(options: {
  ...
  /** @deprecated Alias of langsmithTraceId */
  ...
  const traceId = options.langsmithTraceId ?? options.mlflowTraceId;
  if (options.turnId || traceId) {
    result.metadata = {
      simulator: {
        agentTraceVersion: "1",
        ...(options.turnId ? { turnId: options.turnId } : {}),
        ...(traceId
          ? { langsmithTraceId: traceId, mlflowTraceId: traceId }
          : {})
      }
    };
  }
  ...
  async handleRawBodyAsync(rawBody: string): Promise<{ httpStatus: number; body: string }> {
    ...
    try {
      ...
      const result = buildCompletedTaskResult({
        ...
        turnId: turn.turnId,
        langsmithTraceId: turn.langsmithTraceId ?? turn.mlflowTraceId,
        mlflowTraceId: turn.langsmithTraceId ?? turn.mlflowTraceId
      });
