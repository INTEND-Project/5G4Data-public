...
export class GraphDbTool {
  ...
  /** Infrastructure locality KG: always `intents_and_intent_reports` + infra named graph. */
  static forInfrastructureLookup(
    ...
    const repoBase =
      fallback.repositoryBaseUrl ??
      normalizeSparqlPostEndpoint(fallback.graphDbEndpoint);
